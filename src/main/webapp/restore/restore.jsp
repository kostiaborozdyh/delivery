...
<div class="container">
    ...
    <form method="post" action="/restore">
        <div class="row justify-content-center">
            <div class="col-3">
                <div class="form-group mb-3">
                    <label class="control-label col-xs-4 mb-1"><fmt:message key="enterEmail"/><text style = "color:red;">*</text></label>
                    <input type="text" name="login" class="form-control" required="required">
                    <c:if test = "${sessionScope.invalidM != null}">
                        <text style = "color:red;"><fmt:message key="loginOrEmail"/></text>
                    </c:if>
                    <c:if test = "${sessionScope.invalidLogin != null}">
                        <text style = "color:red;"><fmt:message key="noUserLogin"/></text>
                    </c:if>
                    <c:if test = "${sessionScope.invalidEmail != null}">
                        <text style = "color:red;"><fmt:message key="noUserEmail"/></text>
                    </c:if>
                </div>
            </div>
        </div>
        <div class="row justify-content-center">
            <div class="col-md-auto">
                <p><input type="submit" class="btn btn-success" value="<fmt:message key="restore"/>"></p>
            </div>
        </div>
    </form>
</div>
