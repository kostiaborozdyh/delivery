...
public class FiltrationOrder {
    ...
    public static List<Order> sorting(String sort, List<Order> order){
        ...
        if(sort.equals("sortByMinDateOfArrival"))
            order = order.stream().sorted(Comparator.comparing(Order::getDateOfArrival)).collect(Collectors.toList());
        if(sort.equals("sortByMaxDateOfArrival"))
            order = order.stream().sorted((x,y)->(y.getDateOfArrival().compareTo(x.getDateOfArrival()))).collect(Collectors.toList());
        if(sort.equals("sortByMinCityFrom"))
            order = order.stream().sorted(Comparator.comparing(Order::getCityFrom)).collect(Collectors.toList());
        if(sort.equals("sortByMaxCityFrom"))
            order = order.stream().sorted((x,y)->(y.getCityFrom().compareTo(x.getCityFrom()))).collect(Collectors.toList());
        if(sort.equals("sortByMinCityTo"))
            order = order.stream().sorted(Comparator.comparing(Order::getCityTo)).collect(Collectors.toList());
        if(sort.equals("sortByMaxCityTo"))
